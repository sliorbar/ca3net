...
void _before_run_synapses_4_post_push_spikes()
{
...
double* const _array_synapses_4_delay_1 = _dynamic_array_synapses_4_delay_1.empty()? 0 : &_dynamic_array_synapses_4_delay_1[0];
const size_t _numdelay = _dynamic_array_synapses_4_delay_1.size();
    ///// POINTERS ////////////
        
    double*   _ptr_array_defaultclock_dt = _array_defaultclock_dt;
    int32_t* __restrict  _ptr_array_neurongroup_1__spikespace = _array_neurongroup_1__spikespace;
    double* __restrict  _ptr_array_synapses_4_delay_1 = _array_synapses_4_delay_1;

    std::vector<double> &real_delays = _dynamic_array_synapses_4_delay_1;
    double* real_delays_data = real_delays.empty() ? 0 : &(real_delays[0]);
    int32_t* sources = synapses_4_post.sources.empty() ? 0 : &(synapses_4_post.sources[0]);
    const size_t n_delays = real_delays.size();
    const size_t n_synapses = synapses_4_post.sources.size();
    synapses_4_post.prepare(_n_sources,
                           _n_targets,
