...
double* const _array_synapses_1_delay_1 = _dynamic_array_synapses_1_delay_1.empty()? 0 : &_dynamic_array_synapses_1_delay_1[0];
const size_t _numdelay = _dynamic_array_synapses_1_delay_1.size();
    ///// POINTERS ////////////
    ...
    std::vector<double> &real_delays = _dynamic_array_synapses_1_delay_1;
    double* real_delays_data = real_delays.empty() ? 0 : &(real_delays[0]);
    int32_t* sources = synapses_1_post.sources.empty() ? 0 : &(synapses_1_post.sources[0]);
    const size_t n_delays = real_delays.size();
    const size_t n_synapses = synapses_1_post.sources.size();
    synapses_1_post.prepare(_n_sources,
                           _n_targets,
