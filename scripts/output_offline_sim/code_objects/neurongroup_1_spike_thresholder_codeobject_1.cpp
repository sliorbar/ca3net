...
void _run_neurongroup_1_spike_thresholder_codeobject_1()
{
...
const double spike_th_BC = - 0.034788538807199045;
...
const size_t _numvm = 300;
    ...
    for(size_t _idx=0; _idx<N; _idx++)
    {
        ...
        const double vm = _ptr_array_neurongroup_1_vm[_idx];
        char _cond;
        if(!not_refractory)
            _cond = (vm > spike_th_BC) && false;
        else 
            _cond = (vm > spike_th_BC) && true;
