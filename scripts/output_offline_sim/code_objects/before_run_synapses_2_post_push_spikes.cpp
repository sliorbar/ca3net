...
void _before_run_synapses_2_post_push_spikes()
{
    ...
    ///// CONSTANTS ///////////
    const int64_t _n_sources = 300;
const int64_t _n_targets = 8000;
const size_t _num_source_dt = 1;
const size_t _num_spikespace = 301;
double* const _array_synapses_2_delay_1 = _dynamic_array_synapses_2_delay_1.empty()? 0 : &_dynamic_array_synapses_2_delay_1[0];
const size_t _numdelay = _dynamic_array_synapses_2_delay_1.size();
    ///// POINTERS ////////////
        
    double*   _ptr_array_defaultclock_dt = _array_defaultclock_dt;
    int32_t* __restrict  _ptr_array_neurongroup_1__spikespace = _array_neurongroup_1__spikespace;
    double* __restrict  _ptr_array_synapses_2_delay_1 = _array_synapses_2_delay_1;

    std::vector<double> &real_delays = _dynamic_array_synapses_2_delay_1;
    double* real_delays_data = real_delays.empty() ? 0 : &(real_delays[0]);
    int32_t* sources = synapses_2_post.sources.empty() ? 0 : &(synapses_2_post.sources[0]);
    const size_t n_delays = real_delays.size();
    const size_t n_synapses = synapses_2_post.sources.size();
    synapses_2_post.prepare(_n_sources,
                           _n_targets,
