...
from helper import load_wmx, preprocess_monitors, generate_cue_spikes,\
                   save_vars, save_PSD, save_TFR, save_LFP, save_replay_analysis,save_wmx,save_vars_syn,SynWeightDist, load_spike_trains
...
def learning(spiking_neurons, spike_times, taup, taum, Ap, Am, wmax, w_init, place_cell_ratio):
    """
...
def run_simulation(wmx_PC_E, STDP_mode, cue, save, save_slice, seed, expdesc = None, engine=None, verbose=True, folder=None, expid=None):
    ...
    '''Modified code to accomodate STDP for BC --> PC synapses'''
    dAprebc = w_PC_I*0.01 #1 % change in is allowed
    dApostbc = w_PC_I * 0.01 # Make the stdp symetric
    w_bc_max = w_PC_I * 3 #maximum weight change for inhibitory synapses is three times the default
    taup_bc = taum_bc = 10 * ms # A narrow window for stdp
    
    synapse_setup_bc='''
    w_inh:1
    dAprebc/dt = -Aprebc/taup_bc : 1 (event-driven)
    dApostbc/dt = -Apostbc/taum_bc : 1 (event-driven)
    '''
    #(event-driven)
    on_pre_setup_bc = '''
    x_gaba+=norm_PC_I*w_inh
    Aprebc += dAprebc
    w_inh = clip(w_inh + Apostbc,0,w_bc_max)
    '''
    on_post_setup_bc= '''
    Apostbc += dApostbc
    w_inh = clip(w_inh + Aprebc,0,w_bc_max)
    '''
    #C_PC_I = Synapses(BCs, PCs,synapse_setup_bc, on_pre="x_gaba+=norm_PC_I*w_PC_I", delay=delay_PC_I)
    C_PC_I = Synapses(BCs, PCs,synapse_setup_bc, on_pre=on_pre_setup_bc, on_post=on_post_setup_bc,delay=delay_PC_I)
    C_PC_I.connect(p=connection_prob_BC)
    C_PC_I.w_inh=w_PC_I
    
    '''Adding stdp to pc --> bc synapses'''
    C_BC_E = Synapses(PCs, BCs, on_pre="x_ampa+=norm_BC_E*w_BC_E", delay=delay_BC_E)
    on_pre_setup_pcbc = '''
    x_ampa+=norm_BC_E*w_exc
    Apresyn += dApresyn
    w_exc = clip(w_exc + Apostsyn,0,wmax)'''
    #C_BC_E = Synapses(PCs, BCs,synapse_setup, on_pre=on_pre_setup_pcbc,on_post=on_post_setup,delay=delay_BC_E)
