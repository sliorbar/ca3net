...
if __name__ == "__main__":
    ...
    try:
        ...
        select_Conx = int(sys.argv[4]) if len(sys.argv) > 4 else 1
        pc_ratio = float(sys.argv[5]) if len(sys.argv) > 5 else 0.5
        a_max = float(sys.argv[6]) if len(sys.argv) > 6 else 1.0
    ...
    Ap = Am = 0.02
    wmax_range = (a_max, a_max + 0.2)  # Range for random wmax selection, in nS. Set to a narrow range to ensure reproducibility while allowing for some variability in the results.
    #wmax = 4.0 # 
