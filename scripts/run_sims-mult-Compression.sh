...
for ((i=1; i<=ITERATIONS; i++)); do
  ...
  "$VENV_PY" "$PYTHON_FILE" asym sym "Hom $SYN_Threshold Sweep PC plast A env $i" N 5000 "$SYN_Threshold" 1 A 20000 Y Y
  "$VENV_PY" "$SPIKES_FILE" R B
  "$VENV_PY" "$STDP_FILE" asym sym Y 1
  "$VENV_PY" "$PYTHON_FILE" asym sym "Hom $SYN_Threshold Sweep PC plast B env random order $i" N 5000 "$SYN_Threshold" 2 B 20000 Y Y
  "$VENV_PY" "$SPIKES_FILE" R C
  "$VENV_PY" "$STDP_FILE" asym sym Y 1
  "$VENV_PY" "$PYTHON_FILE" asym sym "Hom $SYN_Threshold Sweep PC plast C env random order $i" N 5000 "$SYN_Threshold" 2 C 20000 Y Y
  "$VENV_PY" "$SPIKES_FILE" R D
  "$VENV_PY" "$STDP_FILE" asym sym Y 1
  "$VENV_PY" "$PYTHON_FILE" asym sym "Hom $SYN_Threshold Sweep PC plast D env random order $i" N 5000 "$SYN_Threshold" 2 D 20000 Y Y
  "$VENV_PY" "$SPIKES_FILE" R E
  "$VENV_PY" "$STDP_FILE" asym sym Y 1
  "$VENV_PY" "$PYTHON_FILE" asym sym "Hom $SYN_Threshold Sweep PC plast E env random order $i" N 5000 "$SYN_Threshold" 2 E 20000 Y N
  #"$VENV_PY" "$PYTHON_FILE" asym sym "Hom $SYN_Threshold No PC plast E env random order $i" N 5000 "$SYN_Threshold" 2 E 5000 Y N

  "$VENV_PY" "$PYTHON_FILE" asym sym "Hom $SYN_Threshold Sweep PC plast preprocess mult env A $i" Y 5000 "$SYN_Threshold" 1 A 1000 N N
  "$VENV_PY" "$PYTHON_FILE" asym sym "Hom $SYN_Threshold Sweep PC plast preprocess mult env B $i" Y 5000 "$SYN_Threshold" 1 B 1000 N N
  "$VENV_PY" "$PYTHON_FILE" asym sym "Hom $SYN_Threshold Sweep PC plast preprocess mult env C $i" Y 5000 "$SYN_Threshold" 1 C 1000 N N
  "$VENV_PY" "$PYTHON_FILE" asym sym "Hom $SYN_Threshold Sweep PC plast preprocess mult env D $i" Y 5000 "$SYN_Threshold" 1 D 1000 N N
  "$VENV_PY" "$PYTHON_FILE" asym sym "Hom $SYN_Threshold Sweep PC plast preprocess mult env E $i" Y 5000 "$SYN_Threshold" 1 E 1000 N N
  "$VENV_PY" "$PYTHON_FILE" asym sym "Hom $SYN_Threshold Sweep PC plast preprocess mult env no cue E $i" N 5000 "$SYN_Threshold" 1 E 10000 N N
