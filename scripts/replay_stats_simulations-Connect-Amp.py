...
def run_simulation(wmx_PC_E,wmx_PC_I, wmx_BC_E, wmx_BC_I, wmx_Conx_PC, STDP_mode, cue, save, save_slice, seed, expdesc=None, engine=None, verbose=True, folder=None, expid=None,  PF_pklf_name=None, PF_pklf_name_postfix=None, 
                   taup_sim=20, taum_sim=20, stdp_post_scale_factor=-0.1, stdp_pre_scale_factor=-0.1, delay_PC_E=2.2, Learning_Rate=0.01,connection_prob_PC = 0.1, connection_prob_BC = 0.25, place_cell_ratio=0.5, select_Conx = 1, connection_prob_BC_E=0.25, STDP_mode_Input = "sym", syn_preserve = 1.0,  tau_inh = 20, stdp_inh_scale_factor = 0.1, inh_max_weight = 2.0, end_duration_length = 10000, wmax = 4.0, do_not_save = "N", env = None, pc_ratio = 0.5):
...
# Synapse plasticity rules for BCs
    ...
    synapse_details = synapse_details + ', Tau_BC_I=' + '{0:.3f}'.format(tau_BC_I) + ', Tau_BC_E=' + '{0:.3f}'.format(tau_BC_E) + ', Tau_PC_I=' + '{0:.3f}'.format(tau_PC_I) + ', wmax_PC_I=' + '{0:.3f}'.format(wmax_PC_I) + ', wmax_BC_E=' + '{0:.3f}'.format(wmax_BC_E) + ', wmax_BC_I=' + '{0:.3f}'.format(wmax_BC_I) + ', inh_plasticity=' + str(inh_plasticity) + ', wmax=' + '{0:.2f}'.format(wmax) + ', synaptic_preserve=' + '{0:.2f}'.format(syn_preserve) + ', end_duration=' + str(end_duration_length) + ', env=' + str(env) + ', pc_ratio=' + '{0:.2f}'.format(place_cell_ratio)
...
if __name__ == "__main__":
    try:
        ...
        pc_ratio = float(sys.argv[13]) if len(sys.argv) > 13 else 0.5
    except:
        STDP_mode = "sym"
        select_Conx = 1
        syn_preserve = 1.0
    ...
    SM_PC, SM_BC, RM_PC, RM_BC, selection, weightmx = run_simulation(
        ...
        taup_sim=taup_sim, taum_sim=taum_sim, stdp_post_scale_factor=stdp_post_scale_factor, 
        stdp_pre_scale_factor=stdp_pre_scale_factor, delay_PC_E=PC_SynDelay, Learning_Rate=Learning_Rate,connection_prob_PC=connection_prob_PC,connection_prob_BC=connection_prob_BC, STDP_mode_Input = STDP_mode_Input, connection_prob_BC_E = connection_prob_BC_E, select_Conx=select_Conx, syn_preserve=syn_preserve, inh_max_weight = inh_max_weight, tau_inh = tau_inh, stdp_inh_scale_factor = stdp_inh_scale_factor, end_duration_length=end_duration_length, wmax=wmax, do_not_save=do_not_save, env = PF_pklf_name_postfix, place_cell_ratio=place_cell_ratio)
