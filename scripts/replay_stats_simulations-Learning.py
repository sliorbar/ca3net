...
import random
#import brian2genn
from helper import load_wmx, load_spike_trains, generate_cue_spikes,\
...
adapt_mult = 3.0 #Adaptation multiplier used for regulating the amount of times PCs spike during replay
...
if __name__ == "__main__":
    ...
    linear = True
    f_in_spikes = "spike_trains_%.1f_linear.npz" % place_cell_ratio if linear else "spike_trains_%.1f.npz" % place_cell_ratio
    f_out = "wmx_%s_%.1f_linear.npz" % (STDP_mode, place_cell_ratio) if linear else "wmx_%s_%.1f.pkl" % (STDP_mode, place_cell_ratio)
    spiking_neurons, spike_times = load_spike_trains(os.path.join(base_path, "files", f_in_spikes))
    # Update folder description for each combination
    expid = datalayerOmen.InitializeTrial(engine=engine, description='Monte Carlo Simulation', 
