...
#PYTHON_FILE="$REPO/scripts/replay_stats_simulations-memory-transfer.py"
PYTHON_FILE="$REPO/scripts/replay_stats_simulations-memory-transfer-CA1.py"
...
STDP_FILE="$REPO/scripts/stdp-Memory_Transfer.py"
SPIKES_FILE="$REPO/scripts/generate_spike_train-Omen.py"

# --- Params ---
ITERATIONS=1
SYN_Threshold="0.1"
# Set to "R" for random (non-ordered) place-field-to-PC assignment, or "N" to keep the
# default ordered assignment (same convention as generate_spike_train-Omen.py / run_sims-mult-Compression.sh).
# CA3 and CA1 are generated as two independent spike trains (own place-field-to-neuron
# assignment and own random seed), so set these independently to give CA3 and CA1 different
# spiking orders.
RANDOM_PF_CA3="R"
RANDOM_PF_CA1="R"
#"$VENV_PY" "$SPIKES_FILE"
...
for ((i=1; i<=ITERATIONS; i++)); do
  ...
  "$VENV_PY" "$SPIKES_FILE" "$RANDOM_PF_CA3" CA3
  "$VENV_PY" "$SPIKES_FILE" "$RANDOM_PF_CA1" CA1 _CA1
  "$VENV_PY" "$STDP_FILE" asym sym N 300 
  "$VENV_PY" "$PYTHON_FILE" asym sym "Mem Trans CA1 random order hSTDP CA3-CA1 $i" alt 5000 "$SYN_Threshold" 1
